...
def create_or_update_employment_log(db: Session, employment_log_input: employment_log_schema.CreateEmploymentLog,
                                    employment_log_id: int = None):
    """Create or update an employment log."""
    if employment_log_id:
        employment_log_data = db.query(model_employment_log.EmploymentLog).filter(
            model_employment_log.EmploymentLog.id == employment_log_id).first()
        if not employment_log_data:
            raise HTTPException(status.HTTP_404_NOT_FOUND,
                                f"Could not update. No employment log found with id: {employment_log_id}")
...
def get_employment_logs(db: Session, applicant_id: int, employment_log_id: int = None):
    ...
    if employment_log_id:
        employment_log = db.query(model_employment_log.EmploymentLog).filter(
            model_employment_log.EmploymentLog.applicant_id == applicant_id,
            model_employment_log.EmploymentLog.id == employment_log_id)
        if not employment_log:
            return HTTPException(status.HTTP_404_NOT_FOUND,
                                 f"No employment log found with id: {employment_log_id} "
                                 f"for applicant id: {applicant_id}")
        return employment_log
    return db.query(model_employment_log.EmploymentLog).filter(
        model_employment_log.EmploymentLog.applicant_id == applicant_id).all()
...
def delete_employment_log(db: Session, employment_log_id: int, applicant_id: int):
    ...
    if not employment_log:
        raise HTTPException(status.HTTP_401_UNAUTHORIZED,
                            f"You are not authorized to modify the employment log with id {employment_log_id}")

    db.query(model_employment_log.EmploymentLog).filter(
        model_employment_log.EmploymentLog.id == employment_log_id).delete()
    db.commit()
