...
async def get_job_positions_by_current_user(db: Session = Depends(get_db_connection), user: user_schema.User = Depends(get_current_user)):
...
async def get_job_position(job_position_id: int, db: Session = Depends(get_db_connection), user: user_schema.User = Depends(get_current_user)):
    employer = employer_service.get_employer_by_user_id(db, user.id)
